...
function getReactionChipMetaText(hideReactionChipMeta: boolean, count: number): string | null {
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-wrap items-end justify-start gap-1">
      {reactionGroups.map(({ key, count, displayChar, emojiName, imageUrl }) => {
        ...
        const chipMetaText = getReactionChipMetaText(hideReactionChipMeta, count);
        return (
          <button
            type="button"
            key={key}
