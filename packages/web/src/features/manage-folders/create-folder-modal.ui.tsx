...
export const CreateFolderModal: React.FC<CreateFolderModalProps> = ({
  ...
  const [backgroundColor, setBackgroundColor] = useState(FOLDER_COLOR_PRESETS[0]!);
  ...
  useEffect(() => {
    if (!open) {
      setName("");
      setBackgroundColor(FOLDER_COLOR_PRESETS[0]!);
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const created = await onCreate({ name: trimmed, backgroundColor });
      if (created) {
        onOpenChange(false);
      }
    } finally {
  ...
  }, [name, backgroundColor, isSubmitting, onCreate, onOpenChange]);
