...
} from "./message-mentions.lib";

describe("injectWorkspaceMentionPlaceholders", () => {
  it("records wildcard mentions without calling resolver", () => {
    const resolve = vi.fn(() => null as number | null);
    const { markdown, tokens } = injectWorkspaceMentionPlaceholders("Hi @**all** there", resolve);
  ...
  it("resolves user id via resolver", () => {
    const { markdown, tokens } = injectWorkspaceMentionPlaceholders("Ping @**John**", (name) =>
      name === "John" ? 42 : null,
    );
    expect(tokens).toHaveLength(1);
  ...
  it("marks unknown users as unresolved", () => {
    const { tokens } = injectWorkspaceMentionPlaceholders("@**Nobody**", () => null);
    expect(tokens[0]?.kind).toBe("unresolved");
  });

  it("uses explicit user id in silent reply @_**Name|id** without calling resolver", () => {
    const resolve = vi.fn(() => null as number | null);
    const { markdown, tokens } = injectWorkspaceMentionPlaceholders(
  ...
  it("parses silent mention inside reply quote block with CRLF", () => {
    const body =
      "@_**Doublek|507** [wrote](https://chat.example.com/#narrow/dm/507/near/5698318):\r\n```quote\r\n:squared_ok:\r\n```\r\n\r\nrest";
    const { tokens, markdown } = injectWorkspaceMentionPlaceholders(body, () => null);
    expect(tokens[0]?.userId).toBe(507);
    expect(tokens[0]?.displayName).toBe("Doublek");
  ...
  it("resolves @_**Name** without id via resolver", () => {
    const { tokens } = injectWorkspaceMentionPlaceholders("@_**Alice**", (n) =>
      n === "Alice" ? 3 : null,
    );
...
describe("restoreWorkspaceMentionPlaceholders", () => {
  it("prefixes wildcard mention label with @", () => {
    const { markdown, tokens } = injectWorkspaceMentionPlaceholders("Hi @**all**", () => null);
    const html = restoreWorkspaceMentionPlaceholders(`<p>${markdown}</p>`, tokens);
    expect(html).toContain(">@all<");
    expect(html).toContain("user-group-mention");
  });

  it("replaces markers with span markup matching sanitizeHtml expectations", () => {
    const { markdown, tokens } = injectWorkspaceMentionPlaceholders("@**John** hi", (n) =>
      n === "John" ? 7 : null,
    );
    const markedLike = `<p>${markdown}</p>`;
    const html = restoreWorkspaceMentionPlaceholders(markedLike, tokens);
