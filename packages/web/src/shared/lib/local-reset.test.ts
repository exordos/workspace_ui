...
function clearTestStorage(): void {
  for (const key of [
    ...
    "messenger-web-sidebar-config",
    "messenger-web-sidebar-config:org-1",
...
describe("performApplicationColdStart", () => {
  ...
  it("preserves auth keys while removing preferences, caches, and misc localStorage", async () => {
    ...
    localStorage.setItem("messenger-web-sidebar-config", '{"activityOpen":true}');
    localStorage.setItem("messenger-web-sidebar-config:org-1", '{"activityOpen":false}');
    ...
    expect(localStorage.getItem("messenger-web-instances")).toBe('[{"id":"1"}]');
    expect(localStorage.getItem("messenger-web-current-instance")).toBe("1");
    ...
    expect(localStorage.getItem("messenger-web-sidebar-config")).toBeNull();
    expect(localStorage.getItem("messenger-web-sidebar-config:org-1")).toBeNull();
