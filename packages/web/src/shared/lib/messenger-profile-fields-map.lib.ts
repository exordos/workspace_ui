/**
 * Maps organization realm custom profile field definitions to semantic slots (phone, job title, …).
 *
...
/** Workspace custom profile field types (see Messenger API /realm/profile_fields). */
const MESSENGER_PROFILE_FIELD_SHORT_TEXT = 1;
const MESSENGER_PROFILE_FIELD_PARAGRAPH = 2;
const MESSENGER_PROFILE_FIELD_DATE = 4;
const MESSENGER_PROFILE_FIELD_PERSON = 6;
...
function matchesPhoneField(f: RealmProfileFieldDefinition): boolean {
  if (f.type !== MESSENGER_PROFILE_FIELD_SHORT_TEXT) return false;
...
function matchesManagerField(f: RealmProfileFieldDefinition): boolean {
  ...
  return (
    f.type === MESSENGER_PROFILE_FIELD_SHORT_TEXT ||
    f.type === MESSENGER_PROFILE_FIELD_PARAGRAPH ||
    f.type === MESSENGER_PROFILE_FIELD_PERSON
  );
}
...
function matchesJobTitleField(f: RealmProfileFieldDefinition): boolean {
  ...
  return (
    f.type === MESSENGER_PROFILE_FIELD_SHORT_TEXT || f.type === MESSENGER_PROFILE_FIELD_PARAGRAPH
  );
}

function matchesBirthdayField(f: RealmProfileFieldDefinition): boolean {
  if (f.type === MESSENGER_PROFILE_FIELD_DATE) return true;
  const n = normName(f.name);
 ...
 */
export function mapMessengerProfileDataToSemanticFields(
  profileData: Readonly<Record<string, { value?: string }>> | undefined,
