/**
 * Client-side Workspace mention markup for Markdown message bodies (`@**Name**`, reply quotes
...
/** Stream/topic wildcard labels (Workspace) — rendered as user-group-mention, not resolved via users store. */
...
export interface WorkspaceMentionToken {
  displayName: string;
 ...
 */
const MESSENGER_MENTION_COMBINED = /@(?:_\*\*(?:([^*|]+)\|(\d+)|([^*|]+))\*\*|\*\*([^*]+)\*\*)/g;
...
function buildMentionSpanHtml(token: WorkspaceMentionToken): string {
  // Visible label: leading `@` + display name (messenger source already has `@**…**`; we mirror that in HTML).
...
/**
 * Replaces Workspace mention Markdown with private-use placeholders and records tokens for HTML substitution.
 */
export function injectWorkspaceMentionPlaceholders(
  markdown: string,
  resolveUserId: (displayName: string) => number | null,
): { markdown: string; tokens: WorkspaceMentionToken[] } {
  const tokens: WorkspaceMentionToken[] = [];
  const regex = new RegExp(MESSENGER_MENTION_COMBINED.source, "g");
  let result = "";
  let lastIndex = 0;
  ...
  while ((m = regex.exec(markdown)) !== null) {
    ...
    let token: WorkspaceMentionToken;
...
/** Substitutes placeholder sequences in rendered HTML with messenger-style mention spans. */
export function restoreWorkspaceMentionPlaceholders(
  html: string,
  tokens: WorkspaceMentionToken[],
): string {
  let out = html;
