...
} from "./messenger-notification-settings.lib";

describe("messenger-notification-settings", () => {
  it("returns defaults when raw is null", () => {
    expect(parseWorkspaceNotificationSettings(null)).toEqual(
      DEFAULT_MESSENGER_NOTIFICATION_SETTINGS,
    );
  });

  it("parses server notification fields", () => {
    const parsed = parseWorkspaceNotificationSettings({
  ...
  it("patches known setting keys", () => {
    const next = patchWorkspaceNotificationSettings(
      DEFAULT_MESSENGER_NOTIFICATION_SETTINGS,
  ...
  it("ignores unknown setting keys", () => {
    ...
    expect(next).toEqual(DEFAULT_MESSENGER_NOTIFICATION_SETTINGS);
  });
});
