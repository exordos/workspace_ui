/**
 * Parses Workspace `GET /users/me` payloads into a normalized current-user record.
 */
...
/** Accepts flat and nested (`user`) Workspace `/users/me` JSON bodies. */
export function parseCurrentUserFromApiData(data: unknown): WorkspaceCurrentUser | null {
  if (!isRecord(data)) {
    return null;
  }
