...
function parseSubscriptionRow(row: unknown): MessengerSubscription | null {
...
export function parseSubscriptions(data: unknown): MessengerSubscription[] | null {
  if (!Array.isArray(data)) {
    return null;
  }
  const parsed: MessengerSubscription[] = [];
