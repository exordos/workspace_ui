/**
 * Workspace auth and server discovery (no active instance required for server_settings).
 */
...
import type { DesktopFlowExchangeResult, MessengerServerSettings } from "./messenger.types";

const log = createLogger("messenger-auth");

interface FetchApiKeyResult {
  // Workspace returns snake_case, so keep the field name as the API uses it.
  api_key: string;
 ...
 * @throws MessengerAuthError on auth or network failure
 */
export async function fetchApiKey(
  ...
  const url = `${base}${env.MESSENGER_API_V1_PATH}/fetch_api_key`;
  ...
  throw new MessengerAuthError(msg || t("auth.invalidLogin"), data.code, data);
}
...
async function exchangeDesktopFlowTokenInRenderer(
  ...
  // Web/PWA path: the renderer calls Workspace itself because there is no Electron main process.
  const base = normalizeRealm(realm);
  const normalizedToken = token.trim();
  if (!base || normalizedToken.length === 0) {
    throw new MessengerAuthError(t("auth.pasteTokenInvalid"));
  }
  ...
  if (response.status >= 400) {
    ...
    throw new MessengerAuthError(message ?? t("auth.pasteTokenInvalid"));
  }
...
async function exchangeDesktopFlowTokenInElectron(
  ...
  if (exchange == null) {
    ...
    throw new MessengerAuthError(t("auth.pasteTokenInvalid"), "DESKTOP_FLOW_BRIDGE_MISSING");
  }
  const result = await exchange({ realm, token });
  if (!result.ok) {
    // The main process already classified the error; here we convert it to a common MessengerAuthError.
    log.error("Electron desktop auth exchange failed", {
      reason: result.reason,
      status: result.status ?? null,
    });
    throw new MessengerAuthError(t("auth.pasteTokenInvalid"), result.reason, result);
  }
  return {
