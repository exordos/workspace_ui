...
import { getCachedOwnAvatarCapabilities } from "./messenger-queue";
import type { WorkspaceOwnAvatarCapabilities } from "./messenger.types";
...
function buildCapabilities(cached: WorkspaceOwnAvatarCapabilities): OwnAvatarCapabilities {
...
export async function uploadOwnAvatar(
  ...
  try {
    ...
    const response = await messengerApi.postFormData("/users/me/avatar", form, options?.signal);
...
export async function removeOwnAvatar(): Promise<AvatarMutationResult> {
  ...
  try {
    ...
    const response = await messengerApi.delete("/users/me/avatar");
    const data = (response.data ?? {}) as AvatarResponsePayload;
