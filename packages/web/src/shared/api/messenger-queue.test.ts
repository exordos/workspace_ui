/**
 * Tests for Messenger API (messenger-queue module).
 */
...
describe("registerQueue", () => {
  it("returns queue_id and last_event_id on success", async () => {
    ...
    expect(mockMessengerApi.post).toHaveBeenCalledWith("/register", {
  ...
  it("parses recent_private_conversations from register payload (messenger array format)", async () => {
    mockMessengerApi.post.mockResolvedValue({
      ok: true,
      status: 200,
      data: {
...
describe("deleteQueue", () => {
  it("uses shared delete transport for the current instance path", async () => {
    ...
    expect(mockRefreshMessengerApiBase).toHaveBeenCalled();
    expect(mockMessengerApi.delete).toHaveBeenCalledWith("/events", { queue_id: "q-123" });
  });
  ...
  it("swallows shared transport errors", async () => {
    mockMessengerApi.delete.mockRejectedValue(new TypeError("Network error"));
