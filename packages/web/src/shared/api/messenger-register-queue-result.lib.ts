...
import { parseRegisterResponseJitsiServerUrl } from "./messenger-register-jitsi.lib";
import {
...
import type {
  ...
  MessengerUserTopic,
  MessengerUserStatusReactionType,
  MessengerUserStatusSnapshot,
  MessengerUserStatusSnapshotEntry,
...
function parseRealmUserGroups(data: unknown): MessengerRealmUserGroup[] | null {
  if (!Array.isArray(data)) {
    return null;
  }
  const parsed: MessengerRealmUserGroup[] = [];
...
function isReactionType(value: unknown): value is MessengerUserStatusReactionType {
  return value === "unicode_emoji" || value === "realm_emoji" || value === "zulip_extra_emoji";
}

function normalizeRegisterUserStatus(
  data: Record<string, unknown>,
): MessengerUserStatusSnapshot | null {
...
function parseUserStatusSnapshot(
...
): MessengerUserStatusSnapshotEntry[] | undefined {
  ...
  const parsed: MessengerUserStatusSnapshotEntry[] = [];
...
export interface RegisterQueueParsedMetadata {
  ...
  realmUserGroups: MessengerRealmUserGroup[] | null;
