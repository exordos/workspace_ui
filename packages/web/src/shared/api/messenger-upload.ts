/**
 * Workspace file uploads (TUS + multipart fallback).
 */
...
async function uploadFileMultipart(
  ...
  const res =
    options?.signal != null
      ? await messengerApi.postFormData("/user_uploads", form, options.signal)
      : await messengerApi.postFormData("/user_uploads", form);
  if (!res.ok) {
...
/** Uploads a file to the messenger API. Uses TUS for large files and multipart fallback otherwise. */
