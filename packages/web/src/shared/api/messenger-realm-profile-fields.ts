/**
 * Workspace GET /realm/profile_fields — custom profile field definitions for the realm.
 *
 * Cached per instance id. Used to interpret `profile_data` keys (job title, manager, person picker).
 */

import { getCurrentInstance, messengerApi } from "~/shared/api/client";
...
export async function fetchRealmProfileFieldDefinitionsWithSignal(
  ...
  try {
    const res = await messengerApi.get("/realm/profile_fields", undefined, signal);
    if (!res.ok) {
