...
describe("getOwnAvatarCapabilities", () => {
  it("returns register-derived capabilities", async () => {
    const { getOwnAvatarCapabilities } = await import("./messenger-avatar-settings");
    getCachedOwnAvatarCapabilitiesMock.mockReturnValue({
...
describe("uploadOwnAvatar", () => {
  it("uploads one file via multipart and returns avatar URL", async () => {
    const { uploadOwnAvatar } = await import("./messenger-avatar-settings");
    getCachedOwnAvatarCapabilitiesMock.mockReturnValue({});
    mockMessengerApi.postFormData.mockResolvedValue({
      ok: true,
      status: 200,
    ...
    expect(mockMessengerApi.postFormData).toHaveBeenCalledWith(
      "/users/me/avatar",
      expect.any(FormData),
      undefined,
    );
...
describe("removeOwnAvatar", () => {
  it("removes avatar and returns fallback avatar URL", async () => {
    const { removeOwnAvatar } = await import("./messenger-avatar-settings");
    ...
    expect(mockMessengerApi.delete).toHaveBeenCalledWith("/users/me/avatar");
  });
