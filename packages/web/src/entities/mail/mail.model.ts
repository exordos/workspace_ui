...
interface MailState {
  ...
  clearSearch: () => void;
  loadMessageAttachments: (uid: string) => Promise<void>;
  saveDraft: (payload: MailComposePayload, uid?: string) => Promise<string | null>;
  sendDraft: (uid: string) => Promise<void>;
  batchMessages: (
    uids: string[],
  ...
  selectMessage: (uid: string, options?: { markSeen?: boolean }) => Promise<void>;
  sendMessage: (payload: MailComposePayload) => Promise<void>;
  deleteMessage: (uid: string) => Promise<void>;
  moveMessage: (uid: string, toFolder: string) => Promise<void>;
  setMessageFlags: (uid: string, patch: MailFlagsPatch) => Promise<void>;
...
export const useMailStore = create<MailState>((set, get) => {
  ...
  async function refreshAfterMessageRemoved(uid: string): Promise<void> {
  ...
  return {
    ...
    hydrateSession() {
      ...
      const session =
        ...
        ({
          ...
          expiresAt: new Date(8640000000000000).toISOString(),
          email: "",
        } satisfies MailSessionInfo);
      logStoreAction("mail", "hydrateSession", { email: session.email });
    ...
    async syncCurrentFolder() {
      ...
      try {
        const result = await apiSyncMailFolder(token, folder);
        set({ messages: result.newMessages });
        await get().loadFolders();
      } catch (error) {
        invalidateSessionIfUnauthorized(error);
      }
