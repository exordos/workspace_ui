...
test.describe("Instance switch @mock", () => {
  test("preserves chat list when switching between instances", async ({ page, messengerApi: _messengerApi }) => {
    ...
    await expect(page).toHaveURL(/\/org\/workspace2\.test\.local\/inbox/, { timeout: 15_000 });

    await page.getByTestId(`instance-quick-${E2E_INSTANCE_ID}`).click();
    await expect(page).toHaveURL(/\/org\/messenger\.test\.local\/inbox/, { timeout: 15_000 });
    await expect(page.getByText("Cached hello")).toBeVisible({ timeout: 15_000 });
  });
});
